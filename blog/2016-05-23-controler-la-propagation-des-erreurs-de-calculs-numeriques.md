...
Lorsque l'on effectue des calculs numériques, ceux-ci sont toujours entachés d'erreurs. C'est le cas notamment lorsque des scientifiques simulent un système physique par des méthodes numériques où une étude mathématique permet de donner une indication sur la précision intrinsèque des méthodes. Par exemple, avec la [méthode d'Euler](https://fr.wikipedia.org/wiki/M%C3%A9thode_d%27Euler), l'erreur par rapport à la solution réelle du problème diminue avec le pas de temps utilisé. Si l'on faisait les calculs numériques à la main, faire tendre ce pas de temps vers $0$ nous permettrait de trouver la solution exacte. Évidemment, il est impossible, même pour une machine, d'avoir un pas de temps de $0$, mais l'on pourrait penser que le réduire autant que notre temps ou puissance de calcul le permet est une bonne chose. *Que nenni !*
